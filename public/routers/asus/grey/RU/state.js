...
function show_banner(L3){// L3 = The third Level of Menu
	...
	if (!dsl_support) {
		banner_code +='<span style="font-family:Verdana, Arial, Helvetica, sans-serif;">Режим работы:</sapn><span class="title_link" style="text-decoration: none;" id="op_link"><a href="Advanced_OperationMode_Content.html" style="color:white"><span id="sw_mode_span" style="text-decoration: underline;"></span></a></span>\n';
	}
	banner_code +='<span style="font-family:Verdana, Arial, Helvetica, sans-serif;">Версия прошивки:</sapn><a href="Advanced_FirmwareUpgrade_Content.html" style="color:white;"><span id="firmver" class="title_link"></span></a>\n';
...
tabtitle[0] = new Array("", "Общие", "WPS", "Мост", "Фильтр MAC-адресов беспроводной сети", "Настройка RADIUS", "Профессионально");
tabtitle[1] = new Array("", "LAN IP", "DHCP-сервер", "Маршруты", "IPTV", "Switch Control");
tabtitle[2] = new Array("", "Подключение", "Dual WAN", "Переключатель портов", "Переадресация портов", "DMZ", "DDNS", "NAT Passthrough", "USB Modem");
tabtitle[3] = new Array("", "Медиасервер", "Сетевое окружение (Samba)", "Общий ресурс в FTP", "Другие настройки");
tabtitle[4] = new Array("", "IPv6");
tabtitle[5] = new Array("", "VPN сервер", "Подробнее о VPN", "VPN Status");
tabtitle[6] = new Array("", "Общие", "Фильтр URL-адресов", "Фильтр ключевых слов", "Фильтр MAC-адресов", "Фильтр сетевых служб");
tabtitle[7] = new Array("", "Режим работы", "Система", "Обновление микропрограммы", "Управление настройками", "Performance tuning", "Настройка DSL");
tabtitle[8] = new Array("", "Общий", "Аренда адресов DHCP", "Беспроводная связь", "Переадресация портов", "Таблица маршрутизации", "Журнал ADSL", "Connections", "IPv6");
tabtitle[9] = new Array("", "Network Analysis", "Netstat", "Wake on LAN");
tabtitle[10] = new Array("", "QoS", "Мониторинг трафика");
tabtitle[11] = new Array("", "Родительский контроль", "Яндекс.DNS");
...
//Level 2 Menu
menuL2_title = new Array("", "Беспроводная сеть", "Локальная сеть", "Интернет", "Приложение USB", "IPv6", "VPN сервер", "Брандмауэр", "Администри- рование", "Системный журнал", "Сетевые утилиты");
...
//Level 1 Menu
menuL1_title = new Array("", "Карта сети", "Гостевая сеть", "Диспетчер трафика", "Родительский контроль", "USB-приложение", "AiCloud", "Дополнительные настройки");
...
function remove_url(){
	...
	if(WebDav_support != -1) {
		tabtitle[3][2] = "Сетевое окружение (Samba) / Cloud Disk";
	}
...
function show_menu(){
	...
	// QIS wizard
	if(sw_mode == 2){
		menu1_code += '<div class="m_qis_r" style="margin-top:-170px;cursor:pointer;" onclick="go_setting(\'/'+ QISWIZARD +'?flag=sitesurvey\');"><table><tr><td><div id="index_img0"></div></td><td><div>Быстрая настройка Интернет</div></td></tr></table></div>\n';
	}else if(sw_mode == 3){
		menu1_code += '<div class="m_qis_r" style="margin-top:-170px;cursor:pointer;" onclick="go_setting(\''+ QISWIZARD +'?flag=lanip\');"><table><tr><td><div id="index_img0"></div></td><td><div>Быстрая настройка Интернет</div></td></tr></table></div>\n'; 	
	}else if(sw_mode == 4){
		menu1_code += '<div class="m_qis_r" style="margin-top:-170px;cursor:pointer;" onclick="go_setting(\'/'+ QISWIZARD +'?flag=sitesurvey_mb\');"><table><tr><td><div id="index_img0"></div></td><td><div>Быстрая настройка Интернет</div></td></tr></table></div>\n';
	}else{
		menu1_code += '<div class="m_qis_r" style="margin-top:-170px;cursor:pointer;" onclick="go_setting(\''+ QISWIZARD +'?flag=detect\');"><table><tr><td><div id="index_img0"></div></td><td><div>Быстрая настройка Интернет</div></td></tr></table></div>\n';
	}	

	// Feature
	menu1_code += '<div class="m0_r" style="margin-top:10px;" id="option0"><table width="192px" height="37px"><tr><td>Общие</td></tr></table></div>\n';
	...
	menu1_code += '<div class="m0_r" style="margin-top:10px;" id="option0">'+'<table width="192px" height="37px"><tr><td>Дополнительные настройки</td></tr></table></div>\n'; 	
...
function Block_chars(obj, keywordArray){
	...
	if(invalid_char != ""){
		alert('Эта строка не может содержать" '+ invalid_char +'" !');
		obj.focus();
		return false;
	}
...
function show_footer(){
	...
	footer_code +='<div align="center" class="copyright">2013  ASUSTeK Computer Inc. Все права защищены.</div><br>';
	...
	if(based_modelid =="RT-N12" || hw_ver.search("RTN12") != -1){	//MODELDEP : RT-N12
		if( hw_ver.search("RTN12HP") != -1){	//RT-N12HP
				...
				footer_code += '<td width="200" id="bottom_help_link" align="left">&nbsp&nbsp<a style="font-weight: bolder;text-decoration:underline;cursor:pointer;" href="http://support.asus.com/download.aspx?SLanguage=en&m=RT-AC66U" target="_blank">Manual</a> | <a style="font-weight: bolder;text-decoration:underline;cursor:pointer;" href="http://support.asus.com/download.aspx?SLanguage=en&m=RT-AC66U" target="_blank">Utility</a></td>';
		}
	}
  ...
  if(swpjverno == ''){
	...
	if (!dsl_support) {
		if(sw_mode == "1")  // Show operation mode in banner, Viz 2011.11
			$("sw_mode_span").innerHTML = "Беспроводной роутер";
		else if(sw_mode == "2")
			$("sw_mode_span").innerHTML = "Режим повторителя";
		else if(sw_mode == "3")
			$("sw_mode_span").innerHTML = "Режим точки доступа (AP)";
		else if(sw_mode == "4")
...
function show_loading_obj(){
	...
	code +='<td><span id="proceeding_main_txt">Подождите, </span><span id="proceeding_txt" style="color:#FFFFCC;"></span></td>\n';
	code +='</tr>\n';
	code +='</table>\n';
...
function reboot(){
	if(confirm("Перезагрузка маршрутизатора займет около 60 секунд. Перезагрузить маршрутизатор?")){
...
function validate_string(string_obj, flag){
	if(string_obj.value.charAt(0) == '"'){
		if(flag != "noalert")
			alert('Эта строка не может начинаться с ["]');
...
function validate_psk(psk_obj){
	...
	if(psk_length < 8){
		alert("Предварительный ключ должен содержать более 8 и менее 63 символов или 64 шестнадцатеричных цифры. Если это поле будет оставлено пустым, в качестве пароля система присвоит значение «00000000».");
	...
	if(psk_length >= 8 && psk_length <= 63 && !validate_string(psk_obj)){
		alert("Предварительный ключ должен содержать более 8 и менее 63 символов или 64 шестнадцатеричных цифры!");
		psk_obj.focus();
		psk_obj.select();
...
function validate_wlkey(key_obj){
	...
	var iscurrect = true;
	var str = "Введите правильный ключ WEP.";
...
function validate_account(string_obj, flag){
	...
	if(string_obj.value.charAt(0) == ' '){
		...
		if(flag != "noalert")
			...
			return 'Эта строка не может начинаться с [&nbsp;&nbsp;&nbsp;]';
	}
	...
	if(invalid_char != ""){
		...
		if(flag != "noalert")
			...
			return "Эта строка не может содержать ' "+invalid_char+" ' !";
	}
	...
	if(sw_mode == 1){
		...
		if((link_status == "2" && link_auxstatus == "0") || (link_status == "2" && link_auxstatus == "2")){
			...
			if(location.pathname == "/" || location.pathname == "/index.html"){
				$("NM_connect_status").innerHTML = "Подключено";
			...
			if(location.pathname == "/" || location.pathname == "/index.html"){
				$("NM_connect_status").innerHTML = '<a style="color:#FFF;text-decoration:underline;" href="/'+ QISWIZARD +'?flag=detect">Отключено</a>';
		...
		if(_wlc_state == "wlc_state=2"){
			...
			$("connect_status").className = "connectstatusoff";
			if(location.pathname == "/" || location.pathname == "/index.html"){
			  $("NM_connect_status").innerHTML = "Отключено";		 
			  $('single_wan').src = "images/New_ui/networkmap/line_dualwan_disconnected.png";
			}	
		}
		$("connect_status").onmouseover = function(){overHint(3);}
...
function isPortConflict(_val){
	if(_val == '80')
		return "Этот порт предназначен для HTTP LAN port.";
	else if(_val == '')
		return "Этот порт предназначен для Download Master.";
	else if(_val == '8082')
		return "Этот порт предназначен для Cloud Disk.";
	else if(_val == '443')
		return "Этот порт предназначен для Cloud Disk.";
	else
		return false;
