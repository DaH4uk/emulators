...
function validateRange(v){
	if(v.value < 0 || v.value >= 256){
		alert(v.value+" is invalid IP address.. \nPlease enter a value between 0to255.");
		v.focus();
		v.select();
		return false;
	}else{
...
function validate_ipaddr_final(o,v){
	if(v == 'wan_ipaddr_x'){
		if(o.value.length == 0){    /*Blank.*/
			alert(o.title+"Fields cannot be blank.");
			...
			document.getElementById(o.name+"_div").style.border = "2px solid #CE1E1E";
			alert(document.form.wan_ipaddr_x.value + " is not a valid IP address!");
			document.form.wan_ipaddr_x1.focus();
			return false;
		}		
			...
			if(document.form.wan_ipaddr_x1.value >= 224){
				alert("Please input the value between 1 to 233.");
				document.form.wan_ipaddr_x1.focus();
				document.form.wan_ipaddr_x1.select();
				return false;
			}
			else if(document.form.wan_ipaddr_x1.value == 127){
				alert(document.form.wan_ipaddr_x1.value + "is kept for localhost address, please input value between 1 to 233.");
				document.form.wan_ipaddr_x1.focus();
				document.form.wan_ipaddr_x1.select();
				return false;
			}
		...
		if(o.value.length == 0){    /*Blank.*/
			...
			if(confirm(o.title+"Fields cannot be blank.\nWe will fulfill correct subnet mask.")){
		...
		if(o.value.length > 0){
			if(!(IP_Validate(o))){ /* IP格式錯誤*/
				...
				alert("You have set WAN IP as RT-AC66U's Gateway, and RT-AC66U maybe could not connect to Internet. Please set Gateway with correct value.");
				document.getElementById(o.name+"_div").style.border = "2px solid #CE1E1E";
				return false;			
			}
		}
