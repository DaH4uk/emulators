...
switch(smallType){
case 'text': {
node.value = String(Value).escape();
node.title = Value;
break;
}
case 'password': {
...
var go_btn = top.document.getElementById("searchButton");
content_len = go_btn.value.length;//alert("content_len "+content_len );
if(content_len >= 7)
...
var width = top.document.getElementById("support").clientWidth + top.document.getElementById("search").clientWidth;//473+325=798
var screen_width = top.document.documentElement.clientWidth;//858
...
function goto_formframe(page,menu)
{
page=page.split(".");
var frameObject= top.document.getElementById("formframe");//alert('frameObject.id ='+frameObject.id+'\nframeObject.src = '+frameObject.src);
if(page[0]=="BAS_basic")
{
frameObject.src = "../html/BAS_basic.htm";
}else{
var url= "../html/"+page[0]+".htm";
frameObject.src =convertURL(url);
}
}
...
{
//var frame_height= window.top.document.getElementById("formframe_div").style.height.replace(/px/,"");
var page_main =document.getElementById("main");//alert("page_main "+ page_main.style.height);
var page_help = document.getElementById("help");//alert("page_help "+ page_help.style.height);
var page_sw_help = document.getElementById("help_switch");//alert("page_sw_help "+ page_sw_help.style.height);
if( help_flag % 2 == 0 )
{
page_main.style.bottom="150px";
page_help.style.display="";
page_sw_help.className="open_help";
}
else
{
page_main.style.bottom="30px";
page_help.style.display="none";
page_sw_help.className="close_help";
}//alert("Fire : "+ frame_height + " : "+ page_main.style.height + " : " + page_help.style.display + " : " + page_sw_help.className);
}
...
if( help_flag % 2 == 0 )
{
page_main.style.height=frame_height-170;//20+30+120
page_help.style.display="";
page_sw_help.className="open_help";
}
else
{
page_main.style.height=frame_height-50;//20+30
page_help.style.display="none";
page_sw_help.className="close_help";
}//alert("1.IE : "+ frame_height + " : " +page_main.style.height + " : " + page_help.style.display + " : " + page_sw_help.className);
}
}
...
{
var frame_height= window.top.document.getElementById("formframe_div").style.height.replace(/px/,"");//frame_height = 707;
if( help_flag % 2 == 0 )
{
page_main.style.height=frame_height-200;//50+30+120
page_help.style.display="";
page_sw_help.className="open_help";
}
else
{
page_main.style.height=frame_height-90;//50+30
page_help.style.display="none";
page_sw_help.className="close_help";
}//alert("2.IE : "+ frame_height + " : " + page_main.style.height + " : " + page_help.style.display + " : " + page_sw_help.className);
}
}
function isIE()
{
...
if(!(isFirefox=navigator.userAgent.indexOf("Firefox")>0)) //判读浏览器为FF不执行以下FF浏览器不支持导致报错的语句
{
document.execCommand('ClearAuthenticationCache');
}
if(top.document.forms[0]==null){
form_name = 'uiLogoutForm';
$('loading').innerHTML = '<form id="uiLogoutForm" method="post" action=""></form>';
}
$H({
'getpage' :'html/logout.htm',
'var:page' : '*',
...
function do_search()
{
//var key = top.document.forms[0].search.value.replace(/ /g,"%20");
var key =top.document.getElementById("searchText");
key = key.value.replace(/ /g,"%20");
