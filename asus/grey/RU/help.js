var Untranslated = {
	...
	go_to_wan_setting : "Go to WAN setting" // need to replace string manually in device-map/internet.html
};
...
function enableMonomode(){
	...
	document.titleForm.current_page.value = "device-map/printer.html";
...
function gotoguestnetwork(){
	top.location.href = "Guest_network.html";
}

function gotocooler(){
	top.location.href = "Advanced_PerformanceTuning_Content.html";
}
// Viz add 2013.04 for dsl sync status
function gotoDSL_log(){
	top.location.href = "Main_AdslStatus_Content.html";
}
...
function overHint(itemNum){
	...
	if(itemNum == 8){
		...
		if(band5g_support != -1){		
				if(wlan0_radio_flag == "0" && wlan1_radio_flag == "0")
						wifiDesc = "<b>2.4GHz -</b><br>Disabled<br><br><b>5 GHz -</b><br>Disabled";
				else if(wlan0_radio_flag == "1" && wlan1_radio_flag == "0")
						wifiDesc = "<b>2.4GHz -</b><br>Enabled<br><br><b>5 GHz -</b><br>Disabled";
				else if(wlan0_radio_flag == "0" && wlan1_radio_flag == "1")
						wifiDesc = "<b>2.4GHz -</b><br>Disabled<br><br><b>5 GHz -</b><br>Enabled";
				else
						wifiDesc = "<b>2.4GHz -</b><br>Enabled<br><br><b>5 GHz -</b><br>Enabled";
	...
	if(itemNum == 3){
		if((link_status == "2" && link_auxstatus == "0") || (link_status == "2" && link_auxstatus == "2")){
			statusmenu = "<div class='StatusHint'>Internet status:</div>";
			statusmenu += "<span>Connected</span>";
		}
		else{
			if(sw_mode == 1){
				if(link_auxstatus == 1)
					statusmenu = "<span class='StatusHint'>Please check ethernet cable is connected properly to the WAN port.</span>";
				else if(link_sbstatus == 1)
					statusmenu = "<span class='StatusHint'>We have detected that your WAN Connection Type is PPPoE. But your PPPoE Setting is not complete.</span>";
				else if(link_sbstatus == 2)
					statusmenu = "<span class='StatusHint'>Invalid account or password, click the link to modify it.</span>";
				else if(link_sbstatus == 3)
					statusmenu = "<span class='StatusHint'>DHCP connection failed.</span>";
				else if(link_sbstatus == 4)
					statusmenu = "<span class='StatusHint'>You have set the wrong dynamic or static IP address for your RT-AC66U.</span>";
				else if(link_sbstatus == 5)
					statusmenu = "<span class='StatusHint'>You have probably stopped the WAN connection manually.</span>";
				else if(link_sbstatus == 6)
					statusmenu = "<span class='StatusHint'>WAN_STOPPED_SYSTEM_ERROR</span>";
				else
					statusmenu = "<span class='StatusHint'>No response from the remote server.</span>";
			}
			else if(sw_mode == 2 || sw_mode == 4){
				if(_wlc_state == "wlc_state=2")
					statusmenu = "<span class='StatusHint'>You have already connected to this AP.</span>";
				else{
					if(_wlc_sbstate == "wlc_sbstate=2")
						statusmenu = "<span class='StatusHint'>Connection failed. Password is not correct. Please click [Rescan] to select another network.</span>";
					else
						statusmenu = "<span class='StatusHint'>The Parent-AP might be turned off. Please check again.</span>";
				}
			}
		}
	}
	...
	// usb storage
	if(itemNum == 2){
		var dmStatus = "Download Master is not installed in your RT-AC66U.";
		if(nodm_support != -1){
		...
		if(foreign_disk_total_mounted_number()[0] == null){
			statusmenu = "<div class='StatusHint'>No USB disk detected.</div>";
		}
			...
			statusmenu = "<span class='StatusHint'>USB function is disabled.</span>";	
		}
		else{
			statusmenu = "<div class='StatusHint'>Download Master:</div>";				
			if(getCookie_help("dm_install") == null || getCookie_help("dm_enable") == null || getCookie_help("hwaddr") != '74:D0:2B:64:F0:B0')
				...
				dmStatus = "Download Master is not installed in your RT-AC66U.";		
				if(nodm_support != -1){
			...
			if(usb_path1 == "usb=modem" || usb_path2 == "usb=modem"){
				statusmenu += "<div class='StatusHint'>USB Adapter:</div>";
...
var usb_path_tmp = new Array('usb=storage', 'usb=printer');
function openHint(hint_array_id, hint_show_id, flag){
	if(hint_array_id == 24){
		...
		if(hint_show_id == 6){	// Viz add 2013.04 for dsl sync status
			...
			statusmenu = "<span class='StatusClickHint' onclick='gotoguestnetwork();' onmouseout='this.className=\"StatusClickHint\"' onmouseover='this.className=\"StatusClickHint_mouseover\"'>Go to the Guest Network</span>";
			_caption = "Guest Network";
		}
		else if(hint_show_id == 3){
			if(sw_mode == 1){
				if((link_status == "2" && link_auxstatus == "0") || (link_status == "2" && link_auxstatus == "2"))
					statusmenu = "<span class='StatusClickHint' onclick='suspendconn(0);' onmouseout='this.className=\"StatusClickHint\"' onmouseover='this.className=\"StatusClickHint_mouseover\"'>Disconnect the Internet connection</span>";
				...
				statusmenu = "<span class='StatusClickHint' onclick='top.location.href=\"http://router.asus.com/QIS_wizard.htm?flag=sitesurvey\";' onmouseout='this.className=\"StatusClickHint\"' onmouseover='this.className=\"StatusClickHint_mouseover\"'>Change the network that RT-AC66U will connect to.</span>";
			}
			else if(sw_mode == 4){
				statusmenu = "<span class='StatusClickHint' onclick='top.location.href=\"/QIS_wizard.htm?flag=sitesurvey_mb\";' onmouseout='this.className=\"StatusClickHint\"' onmouseover='this.className=\"StatusClickHint_mouseover\"'>Change the network that RT-AC66U will connect to.</span>";
			}
			...
			for(i=0; i<foreign_disk_interface_names().length; i++){
				if(foreign_disk_total_mounted_number()[0] == ""){
					statusmenu = "<span class='StatusHint'>No USB disk detected.</span>";
					break;
				}
				...
				if(foreign_disk_total_mounted_number()[i] != "0" && foreign_disk_total_mounted_number()[i] != "" && usb_path_curr != "usb=")
					statusmenu += "<div style='margin-top:2px;' class='StatusClickHint' onclick='remove_disk("+parseInt(foreign_disk_interface_names()[i])+");' onmouseout='this.className=\"StatusClickHint\"' onmouseover='this.className=\"StatusClickHint_mouseover\"'>Eject USB disk <span style='font-weight:normal'>"+ decodeURIComponent(foreign_disks()[i]) +"</span></div>";
				else 
					continue;
			}
			if(current_url!="index.html" && current_url!=""){
				if((usb_path_tmp[0] != usb_path1) && usb_path1 != "usb=" && usb_path1 != "usb=printer")
					statusmenu += "<div style='margin-top:2px;' class='StatusClickHint' onclick='remove_disk(1);' onmouseout='this.className=\"StatusClickHint\"' onmouseover='this.className=\"StatusClickHint_mouseover\"'>Eject USB disk <span style='font-weight:normal'>USB Disk 1</span></div>";
				if((usb_path_tmp[1] != usb_path2) && usb_path2 != "usb=" && usb_path2 != "usb=printer")
					statusmenu += "<div style='margin-top:2px;' class='StatusClickHint' onclick='remove_disk(2);' onmouseout='this.className=\"StatusClickHint\"' onmouseover='this.className=\"StatusClickHint_mouseover\"'>Eject USB disk <span style='font-weight:normal'>USB Disk 2</span></div>";
			}
	...
	if(hint_array_id == 0 && hint_show_id > 21) // for status icon
		...
		return overlib('For more details, please visit <a style="font-weight: bolder;text-decoration: underline;" href="https://support.asus.com" target="_blank">http://support.asus.com</a>', HAUTO, VAUTO);
	else{
...
if (typeof ol_text=='undefined') var ol_text='For more detail, you can also refer to the <a id="faq1" href="" target="_blank" style="font-weight: bolder;text-decoration:underline;">Port forwarding FAQ</a> and <a id="faq2" href="" target="_blank" style="font-weight: bolder;text-decoration:underline;">DMZ FAQ</a>.';
