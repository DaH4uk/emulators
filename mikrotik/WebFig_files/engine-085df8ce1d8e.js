...
function loadGUM(maps, gums, idx) {
    ...
    request('GET', '/webfig/' + gums[idx].unique, null, function(resp) {
        var c = eval('(' + resp + ')');
...
function onLogin(err, autologin) {
    if (err == null) {
        ...
        if (autologin) {
            window.name = 'noautologin=1';
            window.location.replace('/mikrotik/quick_setup.html' + window.location.hash);
        }
        else {
            window.name = 'error=' + err;
            // $(nigga).load("./quick_setup.html");
            window.location.replace('/mikrotik/quick_setup.html' + window.location.hash);
        }
    }
    else {
