...
//				tmp="\u00a0\u00a0"+TA2+":"+"\u00a0<a href=\"http://support.dlink.com.tw/\" onclick=\"return jump_if();\">"+CMOmodel_name+"</a>"; 				
			...
			case 1:				
				...
				break;
			case 2:
				tmp_text=document.createTextNode(sd_FWV+": "+"5.10");
				tmp_td.setAttribute("align","right");
...
function gen_topnav(highlight){
	...
	for (i=1; i<=topnav_text.length;i++){
		if(CMOwlan0_mode=="rt")
			...
			if(chPage) location.href = topnav_href+".htm";
		}
		...
		tmp_td[i].innerHTML="<a href=\""+topnav_href+".htm\" onclick=\"return jump_if();\">"+topnav_text[i-1]+"</a>";
	...
	for (i=0; i<tmpValue.length;i++){
		...
		if(CMOwlan0_mode=="rt" || tmpValue[i][2]) { /* Wireless Mode Type confirm */
			...
			tmp_div[i].setAttribute("id", "sidenavoff"); 
				...
				tmp_div[i].innerHTML="<a href=\""+tmpValue[i][1]+".htm\" onclick=\"return jump_if();\">"+tmpValue[i][0]+"</a>";
		} else { 
